...
import com.example.auth.exception.types.*;
import com.example.auth.constants.AppConstants;
...
public class GlobalExceptionHandler {
    ...
    public final ResponseEntity<ResponseDto> handleBadCredentialException(BadCredentialsException ex) {
        ex.printStackTrace();
        log.error(ex.getMessage());
        return new ResponseEntity<>(new ResponseDto("400","Incorrect email or password!"), HttpStatus.BAD_REQUEST);
    }
    ...
    public final ResponseEntity<ResponseDto> userNotFoundException(UserNotFoundException ex) {
        ex.printStackTrace();
        log.error("Given user email not found!");
        return new ResponseEntity<>(new ResponseDto("500","Given User email Not Found!"), HttpStatus.INTERNAL_SERVER_ERROR);
    }
